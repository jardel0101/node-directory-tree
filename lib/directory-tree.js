...
function directoryTree (path, options, rootPathBoolean, onEachFile, onEachDirectory) {
	if(!rootPath || rootPathBoolean){
		rootPath = path
	}
	...
	if (stats.isFile()) {
		...
		if(path == rootPath){
			item.children = dirData.map(child => directoryTree(PATH.join(path, child), options, false, onEachFile, onEachDirectory)).filter(e => !!e);
		}
